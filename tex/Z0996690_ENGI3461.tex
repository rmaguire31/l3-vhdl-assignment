...
The 32-bit addition is split into 8-bit units which are executed in parallel. In each carry select unit, short ripples adders compute sum candidates for both possible carry in values. Simultaneously, the first intercell carry signal is computed by the inital short ripple adder. The intercell carry signals then propagate through the multiplexors selecting the correct sum and carry out candidates, rather than through the ripple adders.
...
Figure~\ref{fig:hierarchy} illustrates the hierarchy of files used to build the 32-bit CSA in Figure~\ref{fig:csa} and a 32-bit carry ripple adder for comparison. These were implemented in \Altera \QuartusII.
...
Figure~\ref{fig:hierarchy} evidences how the CSA and CRA shared as much VHDL as possible to avoid discrepencies in implementation efficiency. Only the top-level files used the \QuartusII block schematic file format.
